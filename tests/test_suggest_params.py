...
class TestOutputConstraints:
    def test_requested_call_width_drives_peak_and_ring_geometry(self):
        plan = suggest_call_width(
            n_samples=49_770,
            p_list=[639_577],
            gpu_budget_bytes=float(80 * 2**30),
            n_rand_vec=16,
            requested_call_width=8192,
            ring_depth=4,
        )

        assert plan.feasible
        assert plan.call_width == 8192
        assert plan.n_calls == 79
        assert plan.block_bytes == 49_770 * 2048
        assert plan.host_ring_gib < 0.5
...
class TestMultiGRM:
    ...
    def test_projected_core_build_peak_includes_fit_time_pcg_refresh(self, optimizer):
        n = 50_000
        G = 60
        n_covar = 10
        n_rand_vec = 100
        ...
        expected_gib = max(
            ...
            streamer_state + _PLAN._refresh_build_live_bytes(
                n,
                geom,
                n_grm=G,
                rank=k,
                n_covar=n_covar,
                n_rand_vec=n_rand_vec if optimizer == "smile_scoring" else 0,
                partitioned=False,
            ),
        ) / (1024**3)
...
class TestRegression:
    def test_single_grm_state_does_not_reserve_a_second_slq_basis(self):
        assert _PLAN._fit_projected_core_state_bytes(1_000, 1, 16) == (
            _PLAN._projected_core_state_bytes(1_000, 1, 16)
        )
        assert _PLAN._fit_projected_core_state_bytes(1_000, 2, 16) > (
            _PLAN._projected_core_state_bytes(1_000, 2, 16)
        )
    ...
    def test_solve_live_bytes_accounts_for_ai_rhs_width(self):
        ...
        expected = (
            _PLAN._fit_projected_core_state_bytes(n, 80, 16)
            + _PLAN._kvrand_cache_bytes(n, 80, 5)
    ...
    def test_projection_live_bytes_uses_single_column_gpy_and_keeps_kvrand(self):
        n = 1_000
        n_grm = 80
        n_rand_vec = 5
        n_covar = 2
        ...
        got = _PLAN._projection_live_bytes(
            n,
            geom,
            n_grm=n_grm,
            rank=16,
        ...
        expected = (
            _PLAN._fit_projected_core_state_bytes(n, n_grm, 16)
            + _PLAN._kvrand_cache_bytes(n, n_grm, n_rand_vec)
            + geom.inflight_packed_row_bytes * n
            + _PLAN._mat_bytes(n, geom.max_unpack_width)
            + 2.0 * _PLAN._mat_bytes(geom.max_unpack_width, 1)
            + _PLAN._mat_bytes(n, n_covar + 1 + n_rand_vec)
            + _PLAN._mat_bytes(n, n_rand_vec + 1)
            + float(n_grm + 1) * _PLAN._mat_bytes(n, 1)
        )
        assert got == pytest.approx(expected, rel=1e-7)

    def test_slq_live_bytes_keeps_kvrand_cache(self):
        n = 1_000
        n_grm = 80
        n_rand_vec = 5
        slq_samples = 7
        ...
        expected = (
            ...
            + geom.inflight_packed_row_bytes * n
            + _PLAN._mat_bytes(n, geom.max_unpack_width)
            + 2.0 * _PLAN._mat_bytes(geom.max_unpack_width, slq_samples)
            + 3 * _PLAN._mat_bytes(n, 1 + n_rand_vec)
            + 2 * _PLAN._mat_bytes(n, n_grm + 1)
            + _PLAN._mat_bytes(n_grm + 1, n_grm + 1)
            + _PLAN.workspace_layout(n, slq_samples, 30, 256 * 1024**2)["peak_bytes"]
        )
        assert got == pytest.approx(expected, rel=1e-7)
    ...
    def test_width_matches_closed_form(self):
        ...
        assert plan.feasible
        assert plan.gpu_budget_gib > 0.0
        assert plan.smile_w_device_cache_bytes is None
